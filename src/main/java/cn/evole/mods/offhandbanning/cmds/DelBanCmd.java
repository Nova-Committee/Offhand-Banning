 ...
 */
public class DelBanCmd extends CommandBase {
    ...
    public void execute(@Nonnull MinecraftServer server, @Nonnull ICommandSender sender, @Nonnull String[] args) throws CommandException {
        if (sender instanceof EntityPlayerMP) {
            ...
            if (item == null) {
                ...
                Const.configHandler.config.removeBanItems(args[0]);
                Const.configHandler.save();
                player.connection.sendPacket(new SPacketChat(new TextComponentString("成功移除！"), ChatType.SYSTEM));
            }
        }
    }
}
